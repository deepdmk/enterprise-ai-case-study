"""Centralized path configuration for Phase 1 imports.
...
Usage:
    from src.shared.path_config import configure_paths, PHASE1_ROOT, PHASE0_ROOT
...
# Phase 1 root directory (parent of src/)
PHASE1_ROOT = Path(__file__).parent.parent.parent.resolve()

# Phase 0 infrastructure root (sibling directory)
PHASE0_ROOT = PHASE1_ROOT.parent / "phase-0-infrastructure"
...
def configure_paths(force: bool = False) -> bool:
    """
    Configure sys.path for Phase 1 imports.

    Adds the necessary paths to sys.path in the correct order:
    1. Phase 1 root (for config and src imports)
    ...
    # Add Phase 1 root first (for local config imports)
    phase1_str = str(PHASE1_ROOT)
